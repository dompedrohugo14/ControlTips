...
export default function WinRateEvolution() {
  ...
  const fetchWinRateData = async () => {
    try {
      ...
      if (bets && bets.length > 0) {
        const groupedData: { [key: string]: { total: number; wins: number } } = {};

        bets.forEach(bet => {
          const dateKey = bet.bet_date.split('T')[0];

          if (!groupedData[dateKey]) {
            groupedData[dateKey] = { total: 0, wins: 0 };
          }
          groupedData[dateKey].total += 1;
          if (bet.result === 'win') {
            groupedData[dateKey].wins += 1;
          }
        });
        ...
        const chartData: DataPoint[] = Object.entries(groupedData)
          .map(([date, values]) => {
            cumulativeTotal += values.total;
            cumulativeWins += values.wins;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h1 className="text-3xl font-bold text-yellow-500">Evolução da Taxa de Acerto</h1>
          </div>
        ...
        <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-8 shadow-lg">
          {loading ? (
            ...
            <>
              <div className="mb-8">
                <div className="text-sm text-neutral-400 mb-2">Taxa de Acerto Atual</div>
              ...
              <div className="relative h-96 border-l border-b border-neutral-800">
                <svg className="w-full h-full" viewBox={`0 0 ${data.length * 50} 100`} preserveAspectRatio="none">
                  <defs>
                    <linearGradient id="winRateLineGradient" x1="0%" y1="0%" x2="0%" y2="100%">
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4">
                  <div className="text-sm text-neutral-400 mb-1">Taxa Máxima</div>
                  <div className="text-2xl font-bold text-green-500">
                    {maxValue.toFixed(2)}%
                  </div>
                </div>
                <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4">
                  <div className="text-sm text-neutral-400 mb-1">Taxa Mínima</div>
